...
test('URL imports, wallet events, OAuth messages, and AI staging keep their security gates', function (t) {
  ...
  t.ok(oauth.indexOf('assertBffReady()') !== -1 && githubBff.indexOf("authMode !== 'bff-v1'") !== -1, 'frontend fails closed instead of falling back to the legacy token-returning proxy')
  t.ok(githubBff.indexOf('if (!GITHUB_BFF.origin)') !== -1, 'frontend fails closed when the organization BFF origin is missing')
